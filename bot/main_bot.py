...
from utilities import LEVELS, predict_image, check_keys, get_coordinates
...
async def delete_messages(bot):
    try:
        await bot.delete_message(mes[0].chat.id, mes[0].message_id)
        print('aaa')
...
async def quiz_command(update, context):
    ...
    if not dialog_const0:
        print('--------------------------------------------')
        await context.bot.send_message(update.message.chat.id, "Закончите разговор")
        return
    dialog_const0 = False
    ...
    a = await context.bot.send_message(update.message.chat.id, "Чем вы хотите заняться?!!!", reply_markup=keyboard)
    mes[0] = a
...
async def quiz_ans1(call, context):
    ...
    await delete_messages(context.bot)
    ...
    if ans == 'food':
        ...
        mes[0] = await context.bot.send_message(call.callback_query.message.chat.id, message, reply_markup=keyboard)
        return 2
...
async def quiz_ans_final_end(call, context):
    ...
    callback_button = InlineKeyboardButton(text="Сайт места",
                                           url=f"http://localhost:8080/{place}")
    keyboard = InlineKeyboardMarkup([[callback_button]])
